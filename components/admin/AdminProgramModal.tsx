...
export function AdminProgramModal({ open, initial, saving, onClose, onSave }: AdminProgramModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    const next = initial ?? emptyAdminProgram();
    setForm(next);
    setSlugTouched(Boolean(next.slug?.trim()));
    setCtaTouched(Boolean(next.cta_link?.trim()));
  }, [open, initial]);
  ...
  const syncCtaFromSlug = (slug: string, prevCta: string) => {
    if (ctaTouched && !isAutoGeneratedProgramCta(slug, prevCta)) return prevCta;
  ...
  const updateSlugFromTitle = () => {
    ...
    setForm((f) => ({
      ...f,
      slug,
      cta_link: syncCtaFromSlug(slug, f.cta_link),
    }));
  };

  const handleSave = () => {
    const slug = form.slug.trim() || slugFromTitle(form.title) || "program";
    const payload = {
      ...form,
      slug,
      image: form.image.trim() || getProgramFlyerPath(slug),
      cta_link: normalizeProgramCtaLink(slug, form.cta_link),
    };
    onSave(payload);
  };
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <p className="mt-0.5 text-[11px] text-slate-500">
                  Leave blank to auto-generate: /consultation?program=[slug]
                </p>
                <input
                  className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white"
                  value={form.cta_link}
                  ...
                  placeholder={form.slug ? buildProgramCtaLink(form.slug) : "/consultation?program=..."}
                />
              </div>
              <div>
